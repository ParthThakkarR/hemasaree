...
/**
 * Scheduled tasks.
 * NOTE: These only run in long-lived server processes (Docker / self-hosted).
 * On Vercel serverless, cron jobs should be handled via Vercel Cron instead.
 * See: https://vercel.com/docs/cron-jobs
 */

// Only initialise cron in non-serverless environments
if (typeof process !== 'undefined' && process.env.ENABLE_CRON === 'true') {
  ...
  cron.schedule('0 6 * * *', async () => {
    ...
    try {
      ...
      // Log the report (email queue is imported lazily to avoid circular deps)
      const { emailQueue } = await import('./email/emailQueue');
  ...
  console.log('[CRON] Cron tasks disabled (set ENABLE_CRON=true to enable).');
}
